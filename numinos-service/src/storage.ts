...
export interface PersistRecord { key: string; updatedAt: string; result: any }

const DATA_DIR = path.join(process.cwd(), 'numinos-data');
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

// File-based implementations
export async function persistToFile(orgId: string, userId: string, reqHash: string, result: any) {
...
export async function loadFromFile(orgId: string, userId: string, reqHash: string) : Promise<PersistRecord|null> {
  const key = `${orgId||'org'}|${userId}|${reqHash}`;
  const p = path.join(DATA_DIR, key + '.json');
  if (!fs.existsSync(p)) return null;
  try { return JSON.parse(fs.readFileSync(p,'utf-8')) as PersistRecord } catch(e){ return null }
}

export async function listByOrgUserFile(orgId: string, userId: string) : Promise<PersistRecord[]> {
  const prefix = `${orgId}|${userId}`;
  const files = fs.readdirSync(DATA_DIR).filter(f => f.startsWith(prefix));
  return files.map(f => {
    try { return JSON.parse(fs.readFileSync(path.join(DATA_DIR,f),'utf-8')) as PersistRecord } catch(e){ return null }
  }).filter(Boolean) as PersistRecord[];
}

export async function deleteByOrgUserFile(orgId: string, userId: string) {
  const prefix = `${orgId}|${userId}`;
  const files = fs.readdirSync(DATA_DIR).filter(f => f.startsWith(prefix));
...
export async function persistToPostgres(orgId: string, userId: string, reqHash: string, result: any) {
  ...
  try {
    ...
    const up = await client.query(
      `INSERT INTO numinos_graphs (org_id, user_id, req_hash, payload, updated_at)
       VALUES ($1, $2, $3, $4::jsonb, now())
       ON CONFLICT (org_id, user_id, req_hash) DO UPDATE SET payload = EXCLUDED.payload, updated_at = now()`,
      [orgId, userId, reqHash, JSON.stringify(result)]
    );
...
export async function loadFromPostgres(orgId: string, userId: string, reqHash: string) {
  await ensureSchema();
  const client = await getPool().connect();
  try {
    await client.query('BEGIN');
    ...
    return { key: `${orgId}|${userId}|${reqHash}`, updatedAt: r.rows[0].updated_at, result: r.rows[0].payload } as PersistRecord;
  } catch (e) {
    await client.query('ROLLBACK');
    throw e;
  } finally {
...
export async function listByOrgUserPostgres(orgId: string, userId: string) {
  await ensureSchema();
  const client = await getPool().connect();
  try {
    await client.query('BEGIN');
    ...
    await client.query('COMMIT');
    return r.rows.map((row: any) => ({ key: `${orgId}|${userId}|${row.req_hash}`, updatedAt: row.updated_at, result: row.payload } as PersistRecord));
  } catch (e) {
    await client.query('ROLLBACK');
    throw e;
  } finally {
...
export async function deleteByOrgUserPostgres(orgId: string, userId: string) {
  await ensureSchema();
  const client = await getPool().connect();
  try {
    await client.query('BEGIN');
