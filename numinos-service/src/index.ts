...
if (ipAllowlist.length>0) {
  app.use((req:any,res:any,next:any)=>{
    ...
    if (!ipAllowlist.includes(ip)) return res.status(403).json({ message: 'IP not allowed' });
    next();
  });
}
...
const analyzeLimiter = rateLimit({
  windowMs: 5*60*1000,
  max: 60,
  keyGenerator: (req:any) => `${req.ip}:${req.auth?.orgId || req.body.orgId || 'anon'}:${req.auth?.sub || req.body.userId || 'anon'}`,
  standardHeaders: true,
  legacyHeaders: false
});

// Health
app.get('/healthz', (req:any,res:any) => res.json({ ok: true }));
app.get('/readyz', (req:any,res:any) => res.json({ ok: true }));
...
if (process.env.NUMINOS_HOST_PUBLIC_KEY || process.env.NUMINOS_HOST_PUBLIC_KEY_PATH) {
  app.post('/token', requireHostAssertion, (req:any, res:any) => {
    try {
  ...
  app.post('/token', (req:any, res:any) => {
    try {
      ...
      if (!process.env.NUMINOS_HOST_API_KEY || hostKey !== process.env.NUMINOS_HOST_API_KEY) return res.status(403).json({ message: 'forbidden' });
      const t = mintToken({ userId, orgId });
      res.json({ token: t, expires_in: 300 });
    } catch (e:any) {
      res.status(500).json({ message: 'failed to mint token' });
...
app.get('/export', async (req:any, res:any) => {
  ...
  if (!userId || !orgId) return res.status(400).json({ message: 'userId and orgId required' });
  try {
...
app.delete('/data', async (req:any, res:any) => {
  const hostKey = req.headers['x-host-api-key'] || req.headers['x-api-key'];
  if (!process.env.NUMINOS_HOST_API_KEY || hostKey !== process.env.NUMINOS_HOST_API_KEY) return res.status(403).json({ message: 'forbidden' });
  const { userId, orgId } = req.query;
  if (!userId || !orgId) return res.status(400).json({ message: 'userId and orgId required' });
...
function cleanupOld() {
  try{
    if (process.env.NUMINOS_NO_RETENTION === 'true') return;
    ...
    files.forEach((f: any) => {
      try{
        ...
        if (rec.updatedAt && (now - new Date(rec.updatedAt).getTime()) > ms) fs.unlinkSync(p.join(dir,f));
      }catch(e){}
    });
  }catch(e){}
}
...
app.post('/analyze', requireAuth, analyzeLimiter, async (req: any, res: any) => {
  try {
    const parsed = AnalyzeIn.safeParse(req.body);
    if (!parsed.success) {
      const err = parsed.error;
      if ((err.errors as any[]).some((e: any) => e.message === 'payload too large')) {
    ...
    if (err.message === 'LLM_PARSE_ERROR') return res.status(502).json({ message: 'LLM produced invalid JSON' });
...
async function enqueuePending(orgId: string, userId: string, payload: any) {
  try {
...
function reqLog(event: string, obj: any) {
  try { (pino as any)().info({ event, ...obj }); } catch(e){}
}
...
app.post('/journal', allowUnauthJournal ? async (req:any, res:any) => {
  // In unauth mode we fake minimal req.auth
  if (!req.auth) req.auth = { sub: req.body.authorId || 'dev-user', orgId: 'org' };
...
app.get('/journal', allowUnauthJournal ? async (req:any, res:any) => {
  if (!req.auth) req.auth = { sub: req.query.userId || 'dev-user', orgId: 'org' };
} : requireAuth, async (req:any, res:any) => {
  try {
    const orgId = req.auth.orgId || 'org';
    const userId = req.auth.sub;
