...
export async function analyze(payload, opts) {
    ...
    if (mentionsMinorsSexual(combined)) {
        const empty = { nodes: [], edges: [], metrics: { ADI: 0, volatility: 0, polarity: 0 } };
        return { result: empty };
    }
    ...
    // Basic shape check
    const out = { nodes: [], edges: [], metrics: { ADI: 0, volatility: 0, polarity: 0 } };
    if (Array.isArray(parsedJson.nodes))
        out.nodes = parsedJson.nodes.map((n) => ({ ...n }));
    if (Array.isArray(parsedJson.edges))
        out.edges = parsedJson.edges.map((e) => ({ ...e }));
    if (parsedJson.metrics)
        out.metrics = parsedJson.metrics;
    ...
    for (const n of out.nodes) {
        ...
        if (existing) {
            ...
            if (n.valence && existing.valence !== n.valence)
                existing.valence = 'mixed';
            if (!existing.ts && n.ts)
                existing.ts = n.ts;
        }
        else {
