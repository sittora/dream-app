...
export function loadPrivateKey() {
    ...
    if (!fs.existsSync(p))
        throw new Error('private key not found: ' + p);
...
export function loadPublicKey() {
    ...
    if (!fs.existsSync(p))
        throw new Error('public key not found: ' + p);
    return fs.readFileSync(p, 'utf-8');
}
export function mintToken({ userId, orgId, issuer = process.env.NUMINOS_ISS || 'my-app', scopes = ['analyze'] }) {
    const privateKey = loadPrivateKey();
    const payload = { sub: userId, orgId, scope: scopes };
